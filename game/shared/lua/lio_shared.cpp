...
// I/O Output creation
static int lua_CreateIOOutput(lua_State *L) {
...
// I/O Input creation
static int lua_CreateIOInput(lua_State *L) {
	CBaseEntity *pEnt = luaL_checkentity(L, 1);
	const char *szName = luaL_checkstring(L, 2);
...
static int lua_IOConnect(lua_State *L) {
...
// Fire an I/O input
static int lua_FireIOInput(lua_State *L) {
	if (!lua_istable(L, 1)) {
		return 0;
	}
...
static const luaL_Reg io_funcs[] = {
	{"CreateIOOutput", lua_CreateIOOutput},
	{"CreateIOInput", lua_CreateIOInput},
	{"IOConnect", lua_IOConnect},
	{"FireIOInput", lua_FireIOInput},
	{NULL, NULL}
};

LUALIB_API int luaopen_io (lua_State *L) {
	luaL_register(L, LUA_IOLIBNAME, io_funcs);
	return 1;
}
